...
if lsof -ti :"$PORT" &>/dev/null; then
  ...
  while IFS= read -r pid; do
    if ps -p "$pid" -o comm= 2>/dev/null | grep -q 'node'; then
      TEEBOX_PIDS="$TEEBOX_PIDS $pid"
    fi
  done <<< "$PIDS"
  if [ -n "$TEEBOX_PIDS" ]; then
    echo "  ⚠  Port $PORT is in use — restarting Teebox..."
    echo "$TEEBOX_PIDS" | xargs kill 2>/dev/null || true
    sleep 1
  else
    echo ""
    echo "  ✖  Port $PORT is in use by a non-Teebox process."
    echo "     Free the port or set PORT=<number> and try again."
    echo ""
    read -rp "  Press Enter to close..."
    exit 1
  fi
